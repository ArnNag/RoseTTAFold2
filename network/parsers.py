...
def parse_pdb_w_b_factor(filename):
    ...
    return xyz, mask, np.array(idx_s), np.array(b_factors)
...
def read_templates(qlen, ffdb, hhr_fn, atab_fn, templ_to_use=[], offset=0, n_templ=10, random_noise=5.0):
    xyz_t, mask, qmap, t1d, seq, ids = parse_templates_raw(ffdb, hhr_fn, atab_fn, templ_to_use,
                                                           max_templ=max(n_templ, 20))
